...
用法：
    python jobs/run_backtest.py          # 預設 V31 模式
    python jobs/run_backtest.py --v30    # 純 V30 模式
    python jobs/run_backtest.py --v31    # V31 混合模式
    python jobs/run_backtest.py --v33    # V33 低波動
...
def _seed_backtest_rng() -> None:
    """Seed execution slippage only when a reproducible run requests it."""
    seed = os.getenv('BACKTEST_RANDOM_SEED')
    if seed is not None:
        random.seed(seed)
        print(f"🔒 回測亂數 seed: {seed}")
...
STRATEGY_MODE_PRESETS = {
    ...
    'growth_momentum_breakout': V34_MODE_PRESETS,
    'quality_growth': V35_MODE_PRESETS,
}
...
def _resolve_strategy_names(args, strategy_manager=None):
    ...
    for requested_name in requested_names:
        ...
        if normalized_name != 'v30' and hasattr(manager, 'resolve'):
            try:
                normalized_name = manager.resolve(normalized_name, warn_legacy=False)
            except KeyError:
                pass
...
class BacktestEngine:
    ...
    def _apply_execution_slippage(self, price: float, side: str) -> float:
        ...
        return apply_slippage(price, side, max_slippage, random)
    ...
    @property
    def positions(self):
        return self.portfolio.positions if hasattr(self, 'portfolio') else self._compat_positions

    @positions.setter
    def positions(self, value):
        """Compatibility for callers that construct an engine without __init__."""
        if hasattr(self, 'portfolio'):
            self.portfolio.replace_positions(value)
        else:
            self._compat_positions = value
    ...
    def buy(self, stock_id, price, date_str):
        ...
        fee = self.costs.buy_cost(cost)
        
        if self.capital < cost + fee:
            return
        
        self.portfolio.debit(cost + fee)
        stop_loss = price * (1 - self.stop_loss_pct)
        
        self.portfolio.open_position(stock_id, {
            'shares': shares,
    ...
    def sell(self, stock_id, price, date_str, reason):
        ...
        net = revenue - self.costs.sell_cost(revenue)
        
        profit = net - info['total_cost']
        profit_pct = profit / info['total_cost'] * 100
        
        self.portfolio.credit(net)
        ...
        self.portfolio.close_position(stock_id)
    ...
    def run(self, return_metrics=False):
        ...
        if self.trades and self.persist_results:
            ...
            output_path = Config.BACKTEST_TRADES_CSV
        ...
        if self.persist_results:
            ...
            if not df_profit.empty:
                profit_path = Config.BACKTEST_EQUITY_CSV
        ...
        metrics['risk_metrics'] = calculate_metrics(self.daily_assets, self.trades)
...
class PortfolioBacktestEngine:
    ...
    def run_portfolio_backtest(self) -> dict:
        ...
        metrics['risk_metrics'] = calculate_metrics(daily_portfolio_value, all_trades)
        ...
        # 儲存結果
        if self.persist_results:
            Path(Config.BACKTEST_EQUITY_CSV).parent.mkdir(parents=True, exist_ok=True)
