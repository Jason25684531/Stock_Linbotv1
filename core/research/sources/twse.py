...
def classify(response: RawResponse) -> SourceClassification:
    ...
    if stat == "OK":
        tables = response.payload.get("tables")
        if not isinstance(tables, list) or not any(table.get("data") for table in tables if isinstance(table, Mapping)):
            return SourceClassification(
                ResponseKind.EMPTY_RESULT,
                diagnostic_code="W010_source_empty",
                severity="WARN",
                detail="successful response has no tables",
            )
