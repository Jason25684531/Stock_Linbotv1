...
The repository SHALL expose active OpenSpec proposal, design, spec, task, and evidence artifacts as local untracked files through normal `git status`, without force-add or Git tracking, while keeping only generated or archived OpenSpec state ignored.  Any conflict between installed CLI naming validation and project date-prefix policy SHALL be documented with an explicit resolution path.

#### Scenario: New active change is created
- **WHEN** a maintainer creates a new active OpenSpec change
- **THEN** its required artifacts SHALL appear in ordinary Git status, remain absent from `git ls-files`, and require no ignore override
