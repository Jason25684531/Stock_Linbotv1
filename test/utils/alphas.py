...
class Alphas(object):
    ...
    def get_stocks_data(cls, year, list_assets, benchmark):
        ...
        for c in list_assets:
            ...
            df = df.merge(
                bm_data, how="outer", left_on="日期", right_on="benchmark_date"
            )
            list_all.append(df)
    ...
    def get_alpha_methods(cls, self):
        return list(
            filter(
                lambda m: m.startswith("alpha") and callable(getattr(self, m)),
                dir(self),
            )
        )

    @classmethod
    def generate_alpha_single(
        cls, alpha_name, year, list_assets, benchmark, need_save=False
    ):
        # 获取计算因子所需股票数据
