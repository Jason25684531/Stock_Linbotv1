# 這段程式來自 GitHub 上的 yli188/WorldQuant_alpha101_code 專案
# 專案網址: https://github.com/yli188/WorldQuant_alpha101_code
import numpy as np
import pandas as pd
...
def ts_sum(df, window=10):
    ...
    return df.rolling(int(round(window))).sum()
...
def sma(df, window=10):
    ...
    :return: a pandas DataFrame with the time-series min over the past 'window' days.
    """
    return df.rolling(int(round(window))).mean()
...
def stddev(df, window=10):
    ...
    :return: a pandas DataFrame with the time-series min over the past 'window' days.
    """
    return df.rolling(int(round(window))).std()
...
def correlation(x, y, window=10):
    ...
    :return: a pandas DataFrame with the time-series min over the past 'window' days.
    """
    return x.rolling(int(round(window))).corr(y)
...
def covariance(x, y, window=10):
    ...
    :return: a pandas DataFrame with the time-series min over the past 'window' days.
    """
    return x.rolling(int(round(window))).cov(y)
...
def ts_rank(df, window=10):
    ...
    """
    return df.rolling(int(round(window))).apply(rolling_rank)
...
def product(df, window=10):
    ...
    """
    return df.rolling(int(round(window))).apply(rolling_prod)
...
def ts_min(df, window=10):
    ...
    :return: a pandas DataFrame with the time-series min over the past 'window' days.
    """
    return df.rolling(int(round(window))).min()
...
def ts_max(df, window=10):
    ...
    """
    return df.rolling(int(round(window))).max()
...
def delta(df, period=1):
    ...
    """
    return df.diff(int(round(period)))
...
def delay(df, period=1):
    ...
    """
    return df.shift(int(round(period)))
...
def ts_argmax(df, window=10):
    ...
    :return: well.. that :)
    """
    return df.rolling(int(round(window))).apply(np.argmax) + 1
...
def ts_argmin(df, window=10):
    ...
    :return: well.. that :)
    """
    return df.rolling(int(round(window))).apply(np.argmin) + 1
...
class Alphas(object):
    ...
    def alpha007(self):
        ...
        alpha = (
            -1
            * ts_rank(abs(delta(self.close, 7)), 60)
            * sign(delta(self.close, 7)).copy()  # 加入.copy()
        )
    ...
    def alpha009(self):
        ...
        alpha = -1 * delta_close
        alpha[cond_1 | cond_2] = delta_close.copy()  # 加入.copy()
        return alpha
    ...
    def alpha018(self):
        ...
        return -1 * (
            rank(
                (stddev(abs((self.close - self.open)), 5) + (self.close - self.open))
                + df
            )
        )
    ...
    def alpha023(self):
        ...
        alpha = pd.DataFrame(
            np.zeros_like(self.close), index=self.close.index, columns=["close"]
        )
    ...
    def alpha025(self):
        adv20 = sma(self.volume, 20)
        return rank(
            ((((-1 * self.returns) * adv20) * self.vwap) * (self.high - self.close))
        )
        ...
        return ts_min(
            rank(
                rank(
                    scale(
                        log(
                            ts_sum(rank(rank(-1 * rank(delta((self.close - 1), 5)))), 2)
                        )
                    )
                )
            ),
    ...
    def alpha031(self):
        ...
        p1 = rank(
            rank(
                rank(
                    decay_linear(
                        (-1 * rank(rank(delta(self.close, 10)))).to_frame(), 10
                    )
                )
            )
        )
    ...
    def alpha036(self):
        ...
        return (
            ...
            0.6
            * rank(
                (((sma(self.close, 200) / 200) - self.open) * (self.close - self.open))
            )
        )
    ...
    def alpha037(self):
        return rank(
            correlation(delay(self.open - self.close, 1), self.close, 200)
    ...
    def alpha039(self):
        ...
        return (
            ...
            * rank(
                delta(self.close, 7)
                * (1 - rank(decay_linear((self.volume / adv20).to_frame(), 9).CLOSE))
            )
    ...
    def alpha057(self):
        return 0 - (
            ...
            * (
                ...
                / decay_linear(rank(ts_argmax(self.close, 30)).to_frame(), 2).CLOSE
            )
        )
    ...
    def alpha066(self):
        return (
            rank(decay_linear(delta(self.vwap, 4).to_frame(), 7).CLOSE)
            + ts_rank(
                decay_linear(
                    (
                        (
                            ((self.low * 0.96633) + (self.low * (1 - 0.96633)))
                            - self.vwap
                        )
    ...
    def alpha068(self):
        ...
        return (
            ...
            < rank(
                delta(((self.close * 0.518371) + (self.low * (1 - 0.518371))), 1.06157)
            )
        ) * -1
    ...
    def alpha071(self):
        ...
        p1 = ts_rank(
            decay_linear(
                correlation(ts_rank(self.close, 3), ts_rank(adv180, 12), 18).to_frame(),
                4,
            ).CLOSE,
            16,
        )
        p2 = ts_rank(
            decay_linear(
                (
                    rank(((self.low + self.open) - (self.vwap + self.vwap))).pow(2)
                ).to_frame(),
                16,
            ).CLOSE,
            4,
    ...
    def alpha072(self):
        ...
        return rank(
            decay_linear(
                correlation(((self.high + self.low) / 2), adv40, 9).to_frame(), 10
            ).CLOSE
        ) / rank(
            decay_linear(
                correlation(
                    ts_rank(self.vwap, 4), ts_rank(self.volume, 19), 7
                ).to_frame(),
                3,
            ).CLOSE
        )
    ...
    def alpha077(self):
        ...
        p2 = rank(
            decay_linear(
                correlation(((self.high + self.low) / 2), adv40, 3).to_frame(), 6
            ).CLOSE
        )
        df = pd.DataFrame({"p1": p1, "p2": p2})
        ...
        return (
            rank(
                log(
                    product(
                        rank(
                            (rank(correlation(self.vwap, ts_sum(adv10, 50), 8)).pow(4))
                        ),
                        15,
                    )
                )
    ...
    def alpha084(self):
        return pow(
            ts_rank((self.vwap - ts_max(self.vwap, 15)), 21), delta(self.close, 5)
        )
    ...
    def alpha088(self):
        ...
        p2 = ts_rank(
            decay_linear(
                correlation(ts_rank(self.close, 8), ts_rank(adv60, 21), 8).to_frame(), 7
            ).CLOSE,
            3,
        )
        df = pd.DataFrame({"p1": p1, "p2": p2})
    ...
    def alpha092(self):
        ...
        p2 = ts_rank(
            decay_linear(
                correlation(rank(self.low), rank(adv30), 8).to_frame(), 7
            ).CLOSE,
            7,
        )
        df = pd.DataFrame({"p1": p1, "p2": p2})
    ...
    def alpha096(self):
        ...
        p1 = ts_rank(
            decay_linear(
                correlation(rank(self.vwap), rank(self.volume).to_frame(), 4), 4
            ).CLOSE,
            8,
        )
        p2 = ts_rank(
            decay_linear(
                ts_argmax(
                    correlation(ts_rank(self.close, 7), ts_rank(adv60, 4), 4), 13
                ).to_frame(),
                14,
            ).CLOSE,
            13,
    ...
    def alpha098(self):
        ...
        adv15 = sma(self.volume, 15)
        return rank(
            decay_linear(correlation(self.vwap, sma(adv5, 26), 5).to_frame(), 7).CLOSE
        ) - rank(
            decay_linear(
                ts_rank(
                    ts_argmin(correlation(rank(self.open), rank(adv15), 21), 9), 7
                ).to_frame(),
                8,
            ).CLOSE
        )
